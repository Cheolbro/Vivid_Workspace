...
5. 렌더링 완료 후 '최종 Vrew 파일 생성' 버튼 활성화. 클릭 시 파이썬 백엔드가 원본.vrew와 렌더링된 오버레이 영상, 에셋들을 결합하여 최종\_v0.vrew를 생성.
6. 'Vrew 열기' 버튼 활성화. 클릭 시 PC에 설치된 Vrew 프로그램이 실행되며 최종\_v0.vrew 파일이 열림.
7. (수정이 필요한 경우) 사용자는 수정된 기획안을 다시 업로드하고 4~6 과정을 반복할 수 있으며, 이때 파일은 최종\_v1.vrew, v2.vrew 형태로 자동 넘버링되어 저장됨.
